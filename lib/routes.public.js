...
function mountPublicRoutes(app, ctx) {
  ...
  app.get("/api/rsvp", publicLimiter, (req, res) => {
    ...
    // Deliberately does NOT authorize the session. This is a GET, and a GET that
    // grants standing write access is a side effect worth not having; the client
    // already replays the token on the POST, which is the authorization path.
  ...
  app.post("/api/rsvp", publicLimiter, requireCsrfHeader, (req, res) => {
    ...
    let email = has("email") ? cleanLine(body.email, MAX_EMAIL) : undefined;
    ...
    // Skipped when the save changed nothing: re-submitting an identical form is
    // not news to the guest, and it would otherwise let anyone holding one valid
    // code send repeated mail to an address of their choosing.
    if (ctx.sendEmail && saved.email && saved.changed) {
