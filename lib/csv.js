...
/**
 * Spreadsheet formula injection: Excel / Sheets / LibreOffice evaluate a cell
 * whose text begins with one of these, so a guest named `=HYPERLINK(...)` would
 * run when the caterer opens the export. Prefixing with an apostrophe forces
 * the cell to be read as literal text.
 *
 * This is the load-bearing defense — it holds regardless of how the value got
 * into the database (guest form, admin edit, or CSV import).
 */
  ...
  for (const r of rows) {
    if (r.rsvp_id && r.attending === 1 && r.attendees.length > 0) {
      for (const a of r.attendees) {
        out.push({
          invitee: csvSafe(r.name),
          status: "attending",
          guest_name: csvSafe(a.name),
          dietary: csvSafe(a.dietary),
          email: csvSafe(r.email),
          invite_code: csvSafe(r.invite_code),
          message: csvSafe(r.message),
        });
      }
    } else {
      let status = "pending";
...
module.exports = { importInviteesFromCsv, exportResponsesToCsv, canonicalHeader, csvSafe };
