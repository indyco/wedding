...
/**
 * Catch the misconfiguration that costs the most and shows the least: serving a
 * real HTTPS deployment with NODE_ENV unset (or "development").
 *
 * `secure: isProd` on the session cookie keys off NODE_ENV alone, so in that
 * state the admin cookie goes out WITHOUT the Secure flag — it would ride along
 * on any plain-HTTP request to the same host — and the placeholder-secret check
 * is skipped too. Everything still works, which is exactly why it goes unnoticed.
 * An https APP_BASE_URL is the giveaway that this isn't a local run.
 */
function warnIfProdLooking({ isProd, appBaseUrl, warn }) {
  const say = warn || console.warn;
  if (isProd) return false;
  if (!/^https:\/\//i.test(String(appBaseUrl || ""))) return false;
  say(
    `[warn] APP_BASE_URL is ${appBaseUrl} but NODE_ENV is not "production" — ` +
      "the session cookie is being sent without the Secure flag and the " +
      "session-secret strength check is skipped. Set NODE_ENV=production."
  );
  return true;
}
...
function createApp({ store, sendEmail, config = {} } = {}) {
  ...
  warnIfProdLooking({ isProd, appBaseUrl, warn: config.warn });
  ...
  app.use((req, res, next) => {
    ...
    // The guest list is private: names, emails, dietary notes and the RSVP form
    // itself have no business in a search index or an archiving crawler.
    res.setHeader("X-Robots-Tag", "noindex, nofollow, noarchive");
    // HSTS only in production — issuing it from a local http run would pin
    // localhost to https in the developer's browser. Cloudflare usually sets
    // this too; sending it from the origin means it survives a change there.
    if (isProd) {
  ...
  app.post("/api/admin/login", authLimiter, requireCsrfHeader, (req, res, next) => {
    ...
    // Both must be strings. A JSON body can carry an object or a boolean here,
    // and SQLite refuses to bind either — which surfaced as an unauthenticated
    // 500 (plus a stack trace in the log) on a route anyone can reach.
  ...
  app.post("/api/admin/change-credentials", writeLimiter, requireAdmin, requireCsrfHeader, (req, res) => {
    ...
    // Same reason as login: anything non-string here would be stringified into
    // the account (an admin renamed to "[object Object]" can't log back in).
    for (const [label, value] of [["username", newUsername], ["password", newPassword]]) {
      if (value !== undefined && value !== null && typeof value !== "string") {
        return res.status(400).json({ error: `New ${label} must be text` });
      }
    }
  ...
  //
  // body-parser tags a malformed body 400 and an oversized one 413. Those are
  // client mistakes, not server faults: reporting them as 500 both misleads the
  // caller and lets anyone — unauthenticated — spray stack traces into the log.
  // Only a genuine 5xx gets logged.
